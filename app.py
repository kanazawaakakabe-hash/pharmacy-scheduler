...
def calculate_gantt_start_date(today_date):
    ...
    # 現在の曜日が月曜日の場合、次の月曜日まで7日かかります。
    # (7 - today.weekday()) は、現在が日曜(6)なら (7-6)=1 で翌日、土曜(5)なら (7-5)=2 で翌々日の月曜になります。
...
def index():
    ...
    return render_template(
        ...
        process_names_form=process_names_form, # 常にサーバー側の最新リストを渡す
        gantt_fixed_start_date=gantt_fixed_start_date, # ★★★ 追加 ★★★
        request=request # Jinjaでrequest.formにアクセスするために渡す
    )
